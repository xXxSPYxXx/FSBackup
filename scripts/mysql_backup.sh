...
if [ "_$backup_method" = "_full" ]; then
    ...
    ${backup_progdump_path}mysqldump-databases --all --add-drop-table --force $extra_mysqldump_flag --password=$backup_mysqlpassword --user=$backup_mysqluser --host=$backup_mysqlhost |gzip > $backup_path/$backup_name-mysql.gz
    exit
fi
